...
func get_initial_attributes() -> Dictionary:
	...
	return {
		...
		"chain_bonus_jumps": 1 if str(element.get("id", "arcane")) == "electric" else 0,
		"chain_falloff_multiplier": 1.1 if str(element.get("id", "arcane")) == "electric" else 1.0,
		...
		"area_tick_interval_multiplier": float(shape.get("area_tick_interval_multiplier", 1.0)) * (0.84 if str(element.get("id", "arcane")) == "electric" else 1.0),
		"area_damage_multiplier": float(shape.get("area_damage_multiplier", 1.0)),
		"slash_range_multiplier": float(shape.get("slash_range_multiplier", 1.0)),
		"slash_size_multiplier": float(shape.get("slash_size_multiplier", 1.0)),
		"slash_width_multiplier": float(shape.get("slash_width_multiplier", 1.0)),
		"slash_damage_multiplier": float(shape.get("slash_damage_multiplier", 1.0)),
		"slash_arc_multiplier": float(shape.get("slash_arc_multiplier", 1.0)),
		"wave_speed_multiplier": float(shape.get("wave_speed_multiplier", 1.0)),
		"wave_width_multiplier": float(shape.get("wave_width_multiplier", 1.0)),
		"wave_damage_multiplier": float(shape.get("wave_damage_multiplier", 1.0)),
		"wave_lifetime_multiplier": float(shape.get("wave_lifetime_multiplier", 1.0)),
