...
@export var fill_color: Color = Color(0.38, 0.82, 1.0)
@export var charge_color: Color = Color(1.0, 0.86, 0.34)
@export var outline_color: Color = Color(0.88, 0.98, 1.0)
@export var preferred_distance: float = 260.0
@export var too_close_distance: float = 170.0
...
func _draw_enemy_shape(_health_ratio: float) -> void:
	...
	if _state == ShooterState.CHARGE:
		var pulse := 0.5 + 0.5 * sin(float(Time.get_ticks_msec()) * 0.025)
		draw_radius += pulse * 5.0
		draw_color = charge_color
		draw_line(Vector2.ZERO, _aim_direction * radius * 2.4, Color(1.0, 0.86, 0.34, 0.8), 2.5)

	var points := PackedVector2Array([
		Vector2(0.0, -draw_radius),
		Vector2(draw_radius * 0.86, 0.0),
		Vector2(0.0, draw_radius),
		Vector2(-draw_radius * 0.86, 0.0),
	])
